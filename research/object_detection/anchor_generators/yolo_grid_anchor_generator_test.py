...
class YoloGridAnchorGeneratorTest(tf.test.TestCase):
  ...
  def test_construct_single_anchor_grid_1by1(self):
    """Builds a 1x1 anchor grid to test the size of the output boxes."""
    exp_anchor_corners = [[0.0, 0.0, 1.0, 1.0]]
    ## h_min, w_min, h_max, w_max

    box_specs = [1.0, 1.0] # w, h 
    anchor_generator = ag.YoloGridAnchorGenerator(
        box_specs,
        base_anchor_size=tf.constant([1, 1], dtype=tf.float32))
    anchors = anchor_generator.generate(feature_map_shape_list=[(1, 1)])
  ...
  def test_construct_single_anchor_grid_2by2(self):
    """Builds a 1x1 anchor grid to test the size of the output boxes."""
    exp_anchor_corners = [[0.0, 0.0, 0.5, 0.5],
                          [0.0, 0.5, 0.5, 1.0],
                          [0.5, 0.0, 1.0, 0.5],                          
                          [0.5, 0.5, 1.0, 1.0]]
    ## h_min, w_min, h_max, w_max
  ...
  def test_construct_dual_anchor_grid_2by2(self):
    """Builds a 1x1 anchor grid to test the size of the output boxes."""
    exp_anchor_corners = [[0.0, 0.0, 0.5, 0.5],
                          [-0.25, -0.25, 0.75, 0.75],
                          [0.0, 0.5, 0.5, 1.0],
